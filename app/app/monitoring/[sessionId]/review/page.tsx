...
export default async function RecognizedItemsReviewPage({ params }: ReviewPageProps) {
  ...
  const { data: items, error: itemsError } = await supabase
    ...
    .select(
      "id, raw_name, brand, size_text, price_minor, old_price_minor, promo_price_minor, currency, confidence, link_confidence, price_tag_text, product_visible_text, review_reason, position_hint, status, created_at, monitoring_photos(storage_path)",
    )
    .eq("company_id", companyId)
    ...
    <main style={{ display: "grid", gap: "1rem", margin: "3rem auto", maxWidth: 1120, padding: "0 1rem" }}>
      ...
      {items && items.length > 0 ? (
          ...
          {items.map((item) => (
            <article key={item.id} style={{ border: "1px solid #d1d5db", borderRadius: 12, padding: "1rem", display: "grid", gap: "0.75rem" }}>
              <div style={{ display: "grid", gap: "0.25rem" }}>
                ...
                <p style={{ margin: 0, color: "#4b5563" }}>
                  Цена: {formatPrice(item.price_minor, item.currency)} · статус: {item.status} · уверенность: {formatConfidence(item.confidence)} · связь: {formatConfidence(item.link_confidence)}
                </p>
              </div>
        ...
        <section style={{ border: "1px solid #d1d5db", borderRadius: 12, padding: "1rem" }}>
          <p style={{ margin: 0 }}>Пока нет распознанных товаров.</p>
        </section>
      )}
    </main>
  );
