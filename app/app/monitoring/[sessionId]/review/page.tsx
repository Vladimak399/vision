...
const statusFilters: Array<{
  ...
  { key: "all", label: "Все", description: "Без фильтра по статусу" },
  {
    key: "ready",
    label: "Готовые",
    description: "Сопоставлено, OCR подтверждён или Нет в ассортименте",
  },
  {
    key: "problem",
    label: "Проблемные",
    description: "Ошибка OCR или строки без кандидатов",
  },
];
...
const queueFilters: Array<{
...
}> = [
  {
    key: "all",
    label: "Все задачи",
    description: "Без дополнительного фильтра",
  },
  {
    key: "ai_candidate",
    label: "AI candidates",
    description: "Подсказки AI, которые надо принять руками",
  },
  {
    key: "size_risk",
    label: "Риск размера",
    description: "OCR не видит размер, а кандидат размерный",
  },
  {
    key: "missing_own_price",
    label: "Нет нашей цены",
    description: "У кандидата нет нашей цены",
  },
  {
    key: "missing_competitor_price",
    label: "Нет цены конкурента",
    description: "OCR не распознал цену конкурента",
  },
  {
    key: "low_ocr_confidence",
    label: "Низкий OCR",
    description: "Низкая уверенность распознавания",
  },
  {
    key: "low_link_confidence",
    label: "Низкая связь",
    description: "Низкая уверенность связи ценника",
  },
  {
    key: "unmatched",
    label: "Нет в ассортименте",
    description: "Только explicit unmatched",
  },
];

export default async function RecognizedItemsReviewPage({
  params,
  searchParams,
}: ReviewPageProps) {
  const { sessionId } = await params;
  const resolvedSearchParams = searchParams ? await searchParams : {};
  const departmentFilter = parseDepartmentFilter(
  ...
  if (!user)
    redirect(
      `/login?next=/app/monitoring/${encodeURIComponent(sessionId)}/review`,
    );
  ...
  if (
    !["admin", "manager", "reviewer"].includes(membershipResult.membership.role)
  )
  ...
  if (sessionError)
    return (
      <PageError
        message={`Не удалось загрузить сессию: ${sessionError.message}`}
      />
    );
  ...
  if (itemsError)
    return (
      <PageError
        message={`Не удалось загрузить товары: ${itemsError.message}`}
      />
    );
  ...
  const { data: catalogProducts, error: catalogProductsError } = await supabase
    ...
    .select(
      "id, external_sku, name, brand, size_text, own_price_minor, currency, is_active",
    )
    .eq("company_id", companyId)
    .eq("is_active", true)
    .limit(5000)
  ...
  const suggestionsByItemId = buildSuggestionsByItemId(
    allItems,
    catalogProducts ?? [],
  );
  const itemsAfterBasicFilters = filterItemsByStatus(
    filterItemsByCandidates(allItems, suggestionsByItemId, candidateFilter),
    suggestionsByItemId,
    statusFilter,
  );
  const visibleItems = sortItemsForReview(
    filterItemsByQueue(
      itemsAfterBasicFilters,
      suggestionsByItemId,
      queueFilter,
    ),
    suggestionsByItemId,
  ...
  const needsReviewCount =
    (counts.recognized ?? 0) + (counts.needs_review ?? 0);
  const readyCount =
    (counts.matched ?? 0) + (counts.confirmed ?? 0) + (counts.unmatched ?? 0);
  const withoutCandidateCount = countItemsWithoutCandidates(
    allItems,
    suggestionsByItemId,
  );
  ...
  return (
    <main className="page">
      <header className="grid">
        <p className="eyebrow">
          <Link href={`/app/monitoring/${sessionId}`}>Сессия</Link> / Review
        </p>
        <div className="hero">
          <div>
            <h1>Проверка распознанных товаров</h1>
            <p className="lead">
              {session.stores?.name ?? "Магазин"} ·{" "}
              {session.stores?.address ?? "адрес не указан"}. Сначала проверьте
              спорные товары, затем выгрузите Excel.
            </p>
          </div>
          <Link
            className="btn btn-secondary"
            href={`/app/monitoring/${sessionId}/export.xlsx`}
          >
            Экспорт Excel
          </Link>
        </div>
        ...
        <div className="pill-row">
          {departmentFilters.map((filter) => (
            <Link
              key={filter.key}
              href={getReviewHref(sessionId, {
                department: filter.key,
          ...
          {[
            ...
            <Link
              ...
              href={getReviewHref(sessionId, {
                ...
                candidates: filter.key as ReviewCandidateFilter,
                status: statusFilter,
                queue: queueFilter,
              })}
              style={pillStyle(filter.key === candidateFilter)}
            >
              {filter.label}
            </Link>
          ))}
        ...
        <div className="pill-row">
          {statusFilters.map((filter) => (
            <Link
              ...
              style={pillStyle(filter.key === statusFilter)}
            >
              {filter.label}
            </Link>
          ))}
        ...
        <div className="pill-row">
          {queueFilters.map((filter) => (
            <Link
              ...
              title={filter.description}
              style={pillStyle(filter.key === queueFilter)}
            >
              {filter.label}
            </Link>
          ))}
        ...
        <div className="pill-row">
          {Object.entries(counts).map(([status, count]) => (
            <span key={status} style={countPillStyle}>
      ...
      <section className="card soft">
        ...
        <p style={{ margin: "0.35rem 0 0", color: "#4b5563" }}>
          Готово: {readyCount}. Требует проверки: {needsReviewCount}. Без
          кандидатов: {withoutCandidateCount}. Сейчас показано:{" "}
          {visibleItems.length}. Нет в ассортименте считается только по явному
          статусу “Нет в ассортименте”. Отсутствие match не считается
          отсутствием товара.
        </p>
        <p style={{ margin: "0.35rem 0 0", color: "#4b5563" }}>
          Очереди: AI {queueStats.aiCandidate}, риск размера{" "}
          {queueStats.sizeRisk}, нет нашей цены {queueStats.missingOwnPrice},
          нет цены конкурента {queueStats.missingCompetitorPrice}, низкий OCR{" "}
          {queueStats.lowOcrConfidence}, низкая связь{" "}
          {queueStats.lowLinkConfidence}.
        </p>
        <p style={{ margin: "0.35rem 0 0", color: "#4b5563" }}>
          Что проверять первым: без кандидата → риск размера → AI candidates →
          нет нашей цены → низкая уверенность OCR → остальные строки “На
          проверке”.
        </p>
      ...
      {visibleItems.length > 0 ? (
              ...
              <article key={item.id} className="card review-card">
                <div style={{ display: "grid", gap: "0.25rem" }}>
                  ...
                  <p style={{ margin: 0, color: "#4b5563" }}>
                    Цена: {formatPrice(item.price_minor, item.currency)} ·
                    статус: {getStatusLabel(item.status)} · отдел:{" "}
                    {getDepartmentLabel(item.department)} · уверенность:{" "}
                    {formatConfidence(item.confidence)} · связь:{" "}
                ...
                <dl style={{ display: "grid", gap: "0.35rem", margin: 0 }}>
                  ...
                  <Info
                    label="Старая цена"
                    value={formatPrice(item.old_price_minor, item.currency)}
                  />
                  <Info
                    label="Акция"
                    value={formatPrice(item.promo_price_minor, item.currency)}
                  />
                  <Info label="Текст ценника" value={item.price_tag_text} />
                  <Info
                    label="Текст товара"
                    value={item.product_visible_text}
                  />
                ...
                {activeMatch ? (
                  ...
                  <p style={{ color: "#6b7280", margin: 0 }}>
                    Совпадение с каталогом пока не подобрано.
                  </p>
                )}
                <RecognizedItemReviewControls
                  sessionId={sessionId}
                  item={item}
                  suggestions={suggestions}
                />
        ...
        <section className="empty">
          ...
          <p className="muted">
            Смените фильтр или вернитесь в сессию и проверьте статус обработки
            фото.
          </p>
        </section>
      )}
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#4b5563", margin: 0 }}>
        SKU: {product?.external_sku ?? "—"} · бренд: {product?.brand ?? "—"} ·
        размер: {product?.size_text ?? "—"} · наша цена:{" "}
        {formatPrice(
          product?.own_price_minor ?? null,
          product?.currency ?? "RUB",
        )}{" "}
        · score: {formatConfidence(match.score)} · decision: {match.decision}
      </p>
      {match.decision === "ai_review" ? (
        <p style={noticeStyle("info")}>
          Это подсказка AI. Не принято автоматически.
        </p>
      ) : null}
      {sizeRisk ? (
        <p style={noticeStyle("warning")}>
          OCR не распознал размер. Проверь граммовку перед ручным принятием.
        </p>
      ) : null}
      {missingOwnPrice ? (
        <p style={noticeStyle("warning")}>
          У кандидата нет нашей цены. Сравнение уйдёт на проверку.
        </p>
      ) : null}
...
function buildSuggestionsByItemId(
  items: ReviewItem[],
  products: ReviewCatalogProduct[],
) {
...
function getStatusCounts(items: ReviewItem[]) {
  const counts = REVIEW_STATUSES.reduce<Record<string, number>>(
    (accumulator, status) => {
...
function parseDepartmentFilter(
  value: string | undefined,
): ReviewDepartmentFilter {
...
function parseCandidateFilter(
  ...
  return value === "with_candidate" || value === "without_candidate"
    ? value
    : "all";
}
function parseStatusFilter(value: string | undefined): ReviewStatusFilter {
  return value === "all" || value === "ready" || value === "problem"
    ? value
    : "todo";
}
function parseQueueFilter(value: string | undefined): ReviewQueueFilter {
  return queueFilters.some((filter) => filter.key === value)
    ? (value as ReviewQueueFilter)
    : "all";
}

function filterItemsByCandidates(
  items: ReviewItem[],
  suggestionsByItemId: Map<string, ReviewCatalogSuggestion[]>,
  filter: ReviewCandidateFilter,
) {
  if (filter === "all") return items;
  return items.filter((item) =>
    filter === "with_candidate"
      ? hasCandidate(item, suggestionsByItemId)
      : !hasCandidate(item, suggestionsByItemId),
  );
}
...
function filterItemsByStatus(
  ...
  if (filter === "todo")
    return items.filter(
      (item) => item.status === "recognized" || item.status === "needs_review",
    );
  if (filter === "ready")
    return items.filter(
      (item) =>
  ...
  return items.filter(
    (item) =>
      item.status === "rejected" || !hasCandidate(item, suggestionsByItemId),
  );
}

function filterItemsByQueue(
  items: ReviewItem[],
  suggestionsByItemId: Map<string, ReviewCatalogSuggestion[]>,
  ...
  return items.filter((item) => {
    ...
    if (filter === "missing_own_price")
      return isMissingOwnPrice(item, suggestions);
    if (filter === "missing_competitor_price")
      return isMissingCompetitorPrice(item);
...
function sortItemsForReview(
  items: ReviewItem[],
  suggestionsByItemId: Map<string, ReviewCatalogSuggestion[]>,
) {
  return [...items].sort(
    (left, right) =>
      getReviewPriority(left, suggestionsByItemId.get(left.id) ?? []) -
        getReviewPriority(right, suggestionsByItemId.get(right.id) ?? []) ||
...
function getReviewPriority(
  ...
  if (
    (item.status === "needs_review" || item.status === "recognized") &&
    !hasCandidateFromSuggestions(item, suggestions)
  )
    return 0;
  if (
    (item.status === "needs_review" || item.status === "recognized") &&
    isSizeRisk(item, suggestions)
  )
    return 1;
...
function countItemsWithoutCandidates(
  items: ReviewItem[],
  suggestionsByItemId: Map<string, ReviewCatalogSuggestion[]>,
) {
  return items.filter((item) => !hasCandidate(item, suggestionsByItemId))
    .length;
}
function hasCandidate(
  ...
  return hasCandidateFromSuggestions(
    item,
    suggestionsByItemId.get(item.id) ?? [],
  );
}
function hasCandidateFromSuggestions(
  item: ReviewItem,
  suggestions: ReviewCatalogSuggestion[],
) {
  return Boolean(getActiveMatch(item.matches)) || suggestions.length > 0;
}

function countQueueStats(
  items: ReviewItem[],
  suggestionsByItemId: Map<string, ReviewCatalogSuggestion[]>,
): QueueStats {
...
function getReviewBadges(
  ...
  if (!hasText(item.size_text))
    badges.push({ label: "Нет размера OCR", tone: "warning" });
  if (hasSizeCandidate(item, suggestions))
    badges.push({ label: "Размерный кандидат", tone: "info" });
  if (isSizeRisk(item, suggestions))
    badges.push({ label: "Риск размера", tone: "danger" });
  if (isAiCandidate(item)) badges.push({ label: "AI candidate", tone: "info" });
  if (isMissingOwnPrice(item, suggestions))
    badges.push({ label: "Нет нашей цены", tone: "warning" });
  if (isMissingCompetitorPrice(item))
    badges.push({ label: "Нет цены конкурента", tone: "warning" });
  if (!hasCandidateFromSuggestions(item, suggestions))
    badges.push({ label: "Нет кандидата", tone: "danger" });
  if (isLowOcrConfidence(item))
    badges.push({ label: "Низкая уверенность OCR", tone: "warning" });
  if (isLowLinkConfidence(item))
    badges.push({ label: "Низкая связь ценника", tone: "warning" });
  if (item.status === "unmatched")
    badges.push({ label: "Explicit unmatched", tone: "neutral" });
  return badges;
}

function isAiCandidate(item: ReviewItem) {
  return getActiveMatch(item.matches)?.decision === "ai_review";
}
function isSizeRisk(item: ReviewItem, suggestions: ReviewCatalogSuggestion[]) {
  return !hasText(item.size_text) && hasSizeCandidate(item, suggestions);
}
function isMissingOwnPrice(
  item: ReviewItem,
  suggestions: ReviewCatalogSuggestion[],
) {
  const product =
    getActiveMatch(item.matches)?.catalog_products ??
    suggestions[0]?.product ??
    null;
  return Boolean(product) && product?.own_price_minor === null;
}
function isMissingCompetitorPrice(item: ReviewItem) {
  return item.price_minor === null;
}
function isLowOcrConfidence(item: ReviewItem) {
  return (
    Number.isFinite(item.confidence) && item.confidence < LOW_OCR_CONFIDENCE
  );
}
function isLowLinkConfidence(item: ReviewItem) {
  return (
    item.link_confidence !== null &&
    Number.isFinite(item.link_confidence) &&
    item.link_confidence < LOW_LINK_CONFIDENCE
  );
}
function hasSizeCandidate(
  item: ReviewItem,
  suggestions: ReviewCatalogSuggestion[],
) {
  return [
    getActiveMatch(item.matches)?.catalog_products,
    ...suggestions.map((suggestion) => suggestion.product),
  ].some((product) => hasText(product?.size_text ?? null));
}
...
function getReviewHref(
  sessionId: string,
  filters: {
    department: ReviewDepartmentFilter;
    candidates: ReviewCandidateFilter;
  ...
  if (filters.candidates !== "all")
    params.set("candidates", filters.candidates);
...
function getDepartmentLabel(value: string | null) {
  if (value === "products") return "Продукты";
  if (value === "chemistry") return "Химия";
  return "Без отдела";
}
...
function formatPrice(priceMinor: number | null, currency: string | null) {
  return priceMinor === null
    ? "—"
    : `${(priceMinor / 100).toLocaleString("ru-RU", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${currency || "RUB"}`;
}
