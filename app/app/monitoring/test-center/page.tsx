...
type SessionRow = { id: string; created_at: string; status: string; store_id: string | null; stores: { name: string } | null };
...
export default async function MonitoringTestCenterPage() {
  ...
  const { data: sessions } = await supabase
    ...
    .select("id, created_at, status, store_id, stores(name)")
  ...
  return <main style={{ display: "grid", gap: "1rem", margin: "3rem auto", maxWidth: 1120, padding: "0 1rem" }}>
    ...
    <section style={sectionStyle}><h2 style={{ marginTop: 0 }}>AI и окружение</h2><dl><Row label="Vision" value={`${aiConfig.vision.provider} / ${aiConfig.vision.model}`} /><Row label="Text" value={`${aiConfig.text.provider} / ${aiConfig.text.model}`} /><Row label="Fallback" value={`${aiConfig.fallback.provider} / ${aiConfig.fallback.model}`} /><Row label="Vision rescue" value={`${aiConfig.visionRescue.provider} / ${aiConfig.visionRescue.model}`} /></dl>{envNames.map((name) => <p key={name} style={{ margin: "0.25rem 0" }}>{name}: {process.env[name] ? "Да" : "Нет"}</p>)}<p><Link href="/app/ai-diagnostics">AI-диагностика</Link> · <Link href="/app/monitoring">Мониторинг</Link> · <code>docs/monitoring-e2e-test-plan.md</code></p></section>
    ...
    <section style={sectionStyle}><h2 style={{ marginTop: 0 }}>Последние 5 сессий</h2><table style={{ borderCollapse: "collapse", width: "100%" }}><thead><tr><th style={cellStyle}>Дата</th><th style={cellStyle}>Магазин</th><th style={cellStyle}>Статус</th><th style={cellStyle}>Фото</th><th style={cellStyle}>OCR jobs</th><th style={cellStyle}>Items</th><th style={cellStyle}>Failed jobs/photos</th></tr></thead><tbody>{(sessions ?? []).map((session) => <tr key={session.id}><td style={cellStyle}><Link href={`/app/monitoring/${session.id}`}>{formatDateTime(session.created_at)}</Link></td><td style={cellStyle}>{session.stores?.name ?? "—"}</td><td style={cellStyle}>{session.status}</td><td style={cellStyle}>{countBySession(photoRows, session.id)}</td><td style={cellStyle}>{countBySession(jobRows, session.id)}</td><td style={cellStyle}>{countBySession(itemRows, session.id)}</td><td style={cellStyle}>{countBySession(jobRows.filter((r) => r.status === "failed"), session.id)} / {countBySession(photoRows.filter((r) => r.status === "failed"), session.id)}</td></tr>)}</tbody></table></section>
