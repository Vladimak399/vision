...
export default async function CatalogPage() {
  ...
  try {
    products = await getCatalogProducts(membershipResult.membership.companyId);
  } catch (e) {
    ...
    <main style={{ display: "grid", gap: "1.5rem", margin: "2rem auto", maxWidth: 960, padding: "0 1rem" }}>
      ...
      {canManageCatalog ? (
        ...
        <section style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: "1rem", background: "#f8fafc" }}>
          <h2 style={{ marginTop: 0 }}>Режим просмотра</h2>
          <p style={{ marginBottom: 0, color: "#64748b" }}>
            У вашей роли нет прав на добавление и импорт товаров. Каталог доступен только для просмотра.
          </p>
        </section>
      )}
      ...
      <section style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: "1.5rem" }}>
        ...
        {products.length === 0 ? (
          <p style={{ color: "#64748b", marginBottom: 0 }}>Товаров в каталоге нет.</p>
        ) : (
