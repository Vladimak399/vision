...
const OLD_LABELS = ["старая", "стар.", "стар", "было", "обычная", "обычн.", "обычн", "зачерк", "перечерк"];
const PROMO_LABELS = ["новая", "нов.", "нов", "акция", "акц", "скидка", "скид", "спец", "sale", "promo", "промо"];
...
function classifyCandidate(contextBefore: string, contextAfter: string): LocalPriceParserCandidateKind {
  const oldBefore = lastKeywordIndex(contextBefore, OLD_LABELS);
  const promoBefore = lastKeywordIndex(contextBefore, PROMO_LABELS);

  if (oldBefore >= 0 || promoBefore >= 0) {
    return promoBefore > oldBefore ? "promo" : "old";
  }
...
function lastKeywordIndex(text: string, keywords: string[]): number {
  const normalized = text.toLowerCase();
  return keywords.reduce((last, keyword) => Math.max(last, normalized.lastIndexOf(keyword)), -1);
}
