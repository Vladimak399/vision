...
export function createSupabaseEvidenceClientFromEnv(
  ...
  if (!url) {
    return failure("missing_supabase_url", `${SUPABASE_EVIDENCE_URL_ENV} is required.`, diagnostics);
  }

  if (!isValidUrl(url)) {
    return failure("invalid_supabase_url", `${SUPABASE_EVIDENCE_URL_ENV} must be a valid URL.`, diagnostics);
  }
  ...
  return {
    ok: true,
    client: adaptSupabaseClient(client),
    diagnostics,
  };
}
...
function buildDiagnostics(input: {
  env: SupabaseEvidenceClientFactoryEnv;
  useServiceRole: boolean;
}): SupabaseEvidenceClientFactoryDiagnostics {
  return {
...
function failure(
  code: SupabaseEvidenceClientFactoryError["code"],
  message: string,
  diagnostics: SupabaseEvidenceClientFactoryDiagnostics,
): SupabaseEvidenceClientFactoryResult {
  return {
