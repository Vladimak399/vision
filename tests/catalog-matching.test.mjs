...
const { getCatalogMatchCandidates } = require('../.tmp/catalog-matching-test/catalog-matching.js');
...
function decision(candidate, second) {
  ...
  const review = candidate.reasons.some((r) => r.endsWith('_review'));
  if (!review && candidate.score >= 0.9 && (!second || candidate.score - second.score >= 0.08)) return 'auto';
  return 'suggested';
}
